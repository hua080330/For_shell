...
# 配置文件路径
$script:configDir = Join-Path $env:USERPROFILE ".qiusuo"
...
function Switch-Repo {
    ...
    if ($script:repoSources.ContainsKey($RepoName)) {
        ...
        Write-ErrorMsg "Repository '$RepoName' not found" "Use 'list repos' to see available repositories, or 'add repo' to add a new one"
    }
}
...
function Add-Repo {
    ...
    if ($RepoUrl -match 'github\.com/([^/]+)/([^/]+)') {
        ...
        Write-SuccessMsg "Repository '$repoId' added"
...
function Show-Repos {
    Write-Host "`n[*] Available Repositories:" -ForegroundColor Cyan
...
function Search-Files {
    ...
    # Case 1: Search by MagNo. (6-digit number like "000123")
    ...
    # Case 2: Search by filename (simple keyword or wildcard)
    else {
...
function Download-File {
    ...
    # Check for incomplete download (resume)
    $resumeMode = $false
...
function Get-TabCompletion {
    ...
    $knownCommands = @("list", "ls", "search", "src", "download", "dl", "bg", "jobs", "fg", "stop", "kill", "info", "set", "show", "save", "use", "add", "muldl", "clear", "cls", "help", "?", "exit", "quit")
...
----------------------------------------------------------------------------------------
 ...
 use repo <name>  -           Switch to another repository
 add repo <url>   -           Add a new repository source
...
function Start-InteractiveShell {
    ...
    while ($true) {
        $proxyShort = if ($script:config.PROXY_INDEX) { $script:config.PROXY_INDEX } else { 1 }
        $prompt = "QS[$proxyShort]> "

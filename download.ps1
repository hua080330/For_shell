...
#region SSL/TLS Configuration (方案一：忽略 SSL 证书验证)
add-type @"
using System.Net;
...
Write-Host "[SSL] SSL/TLS: Certificate validation bypassed" -ForegroundColor Yellow
#endregion

#region Encoding Configuration (方案一：设置控制台编码为 UTF-8)
try {
    [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
    [Console]::InputEncoding = [System.Text.Encoding]::UTF8
    $PSDefaultParameterValues['*:Encoding'] = 'utf8'
    Write-Host "[ENCODING] UTF-8 encoding enabled for Chinese support" -ForegroundColor Yellow
} catch {
...
# 修改 Show-FileList 函数，移除分页，直接显示所有文件
function Show-FileList {
    ...
    Write-Host "`n[*] Available Files ($($filtered.Count) total):" -ForegroundColor Cyan
    Write-Host ("=" * 80) -ForegroundColor DarkGray
    
    $index = 1
    foreach ($file in $filtered) {
        $magNo = $index.ToString("000000")
...
---------------------------------------------------------------------------------------
list        ls        Fetch and display file list (all files, no pagination)
search      src       Search by MagNo. (000001) or filename keyword (supports Chinese)
download    dl        Download file: dl 3, dl 3 8 (8 threads), dl 1-5 (batch)
download    dl        Download by filename: dl myfile.apk
bg dl                - Background download: bg dl 3
...
function Start-InteractiveShell {
    ...
    while ($true) {
        ...
        if ($script:commandHistory.Count -gt 100) {
            $script:commandHistory = $script:commandHistory[-100..-1]
        }
        $script:historyIndex = $script:commandHistory.Count
        ...
        switch ($cmd) {
            ...
            "ls" {
                if ($script:fileCache) {
                    ...
                    Write-ErrorMsg "No file list. Run 'list' first." "Run 'list' to fetch files"
                }
            }
            ...
            "download" {
                ...
                $threads = -1
                $target = $dlParts[0]
                if ($dlParts.Count -gt 1) {
                    $threads = [int]$dlParts[1]
                }
                if ($target -match '^\d+-\d+$') {
                    Download-Batch -Range $target -CustomThreads $threads
                } elseif ($target -match '^\d+$') {
                    Download-File -Index ([int]$target) -CustomThreads $threads
                } else {
                    Download-File -FileName $target -CustomThreads $threads
                }
            }
            "dl" {
                $dlParts = $args -split '\s+'
            ...
            "bg" {
                ...
                if ($bgParts[0] -eq "dl") {
                    ...
                    if ($target -match '^\d+$') {
                        Start-BackgroundDownload -Index ([int]$target) -CustomThreads $threads
                    } else {
                        Start-BackgroundDownload -FileName $target -CustomThreads $threads
                    }
                } else {
                    Write-ErrorMsg "Usage: bg dl <index or filename>" "Example: bg dl 3"
                }
            }
            "jobs" { Show-Jobs }
            "fg" {
                if ($args -match '^\d+$') {
                    Foreground-Job -JobId ([int]$args)
                } else {
                    Write-ErrorMsg "Usage: fg <job_id>" "Example: fg 1"
                }
            }
            "stop" {
                if ($args -match '^\d+$') {
                    Stop-JobById -JobId ([int]$args)
                } else {
                    Write-ErrorMsg "Usage: stop <job_id>" "Example: stop 1"
                }
            }
            "kill" {
                if ($args -match '^\d+$') {
                    Kill-JobById -JobId ([int]$args)
                } else {
                    Write-ErrorMsg "Usage: kill <job_id>" "Example: kill 1"
                }
            }
            ...
            "set" {
                ...
                if ($setParts.Count -lt 2) {
                    ...
                    Set-ConfigItem -Key $setParts[0] -Value $setParts[1]
                }
            }
            "show" {
                if ($args -eq "options" -or $args -eq "") {
                    Show-Config
                } else {
                    Write-ErrorMsg "Usage: show options" "Type 'show options' to display configuration"
                }
            }
            "save" {
            ...
            "clear" { 
                Show-Banner
                if ($script:fileCache) {
                    Write-InfoMsg "File list loaded. Use 'list' to refresh."
                }
            }
            "cls" {
                Show-Banner
            ...
            "?" { Show-Help }
            "exit" { 
                Write-SuccessMsg "Exiting... Goodbye!"
            ...
            default {
                Write-ErrorMsg "Unknown command: $cmd" "Type 'help' or '?' for available commands"
            }
        }
        Check-Jobs
    }
